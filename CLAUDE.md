...
`annotated-law` automates plain-language summaries of legislation with citations to primary sources. Phase 0 is a UK tenancy vertical slice: ingest statutes from legislation.gov.uk, build an IR, apply ontology concepts, generate summaries, and serve via Astro. Full plan at `~/.claude/plans/snuggly-wobbling-melody.md`.

## Stack

TypeScript end-to-end, bun runtime, tsgo (`@typescript/native-preview`) for typechecking, oxlint for linting, Astro for the frontend.
